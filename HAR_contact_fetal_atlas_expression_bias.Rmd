...
DE_genes_HAR_targets <- read.table("./contact_lists/DEGs_diff_enrichment_full_trunc.txt", row.names = NULL)
colnames(DE_genes_HAR_targets) <- c("Gene", "DHM", "DEG")
DE_genes_HAR_targets <- DE_genes_HAR_targets %>% filter(DEG == "Human-biased_expression") %>% select(Gene) %>% unique()
...
NSC_DEG_contacts_atlas_avg_exp <- AverageExpression(fetal_human_atlas_seurat_progs,
                                                     features = DE_genes_HAR_targets$Gene, slot = "scale.data")
...
NSC_DEG_contacts_atlas_avg_norm_exp <- AverageExpression(fetal_human_atlas_seurat_progs,
                                                     features = unique(DE_genes_HAR_targets$Gene), slot = "data")
